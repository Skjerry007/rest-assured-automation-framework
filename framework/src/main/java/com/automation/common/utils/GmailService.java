...
import com.automation.common.secretmanager.GmailSecretManager;
        ...
        if (service == null) {
            ...
            try {
                ...
                if (credentialsJson != null && !credentialsJson.trim().isEmpty() && !credentialsJson.contains("your-actual-project-id")) {
                    ...
                    LoggerUtil.info("Successfully loaded Gmail credentials from Google Secret Manager");
                } else {
                    LoggerUtil.warn("Secret Manager returned empty or default project ID credentials. Falling back to local file.");
                    reader = new InputStreamReader(new FileInputStream(CREDENTIALS_FILE_PATH));
                }
            } catch (Exception e) {
                LoggerUtil.warn("Failed to load credentials from Google Secret Manager ({}). Falling back to local file.", e.getMessage());
                reader = new InputStreamReader(new FileInputStream(CREDENTIALS_FILE_PATH));
            }
